...
namespace ApiGameCatalogue.Repositories 
{
  public class GameSqlServerRepository : IGameRepository 
  {
    ...
    public async Task<List<Game>> Get(int page, int quantity) 
    {
      ...
      var command = $"select * Games order by Id offset {((page - 1) * quantity)} rows fetch next {quantity} rows only";
    ...
    public async Task<Game> Get(Guid id)
    {
      ...
      var command = $"select * Games where Id = {id}";
    ...
    public async Task<List<Game>> Get(string name, string producer) 
    {
      ...
      var command = $"select * Games where Name = '{name}' and Producer = '{producer}'";
    ...
    {
      var command = $"insert into Games (Id, Name, Producer, Price) values ('{game.Id}', '{game.Name}', '{game.Producer}', {game.Price.ToString().Replace(",", ".")})";
